...
public class AiAgent extends Task implements RunnableTask<AiAgent.Output> {
    ...
    @Schema(
        ...
        description = """
            ...
            When enabled, it will store the memory inside a K/V pair, be careful that if your internal storage implementation didn't support expiration, the K/V pair may exist forever even if you set a TTL inside the Memory."""
    )
    @Builder.Default
    private Memory memory = Memory.builder().build();

    @Override
    public Output run(RunContext runContext) throws Exception {
        ChatMemory chatMemory;
        boolean memoryIsEnabled = memory != null && runContext.render(memory.getEnabled()).as(Boolean.class).orElse(false);
        if (memoryIsEnabled) {
            // first, check if we have persisted a memory and recover it
            String memoryId = runContext.render(memory.getMemoryId()).as(String.class).orElseThrow();
            chatMemory = MessageWindowChatMemory.withMaxMessages(runContext.render(memory.getMessages()).as(Integer.class).orElseThrow());
            KVStore kvStore = runContext.namespaceKv(runContext.flowInfo().namespace());
            if (kvStore.exists(memoryId)) {
                KVValue kvValue = kvStore.getValue(memoryId).orElseThrow();
                List<ChatMessage> messages = ChatMessageDeserializer.messagesFromJson((String) kvValue.value());
        ...
        try {
            ...
            if (memoryIsEnabled) {
                // store the memory in KV for future usage
                String memoryId = runContext.render(memory.getMemoryId()).as(String.class).orElseThrow();
                KVStore kvStore = runContext.namespaceKv(runContext.flowInfo().namespace());
                String memoryJson = ChatMessageSerializer.messagesToJson(chatMemory.messages());
                KVValueAndMetadata kvValueAndMetadata = new KVValueAndMetadata(null, memoryJson); // TODO expiration
