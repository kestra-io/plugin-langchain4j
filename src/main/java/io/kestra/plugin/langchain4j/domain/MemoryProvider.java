...
public abstract class MemoryProvider extends AdditionalPlugin {
    ...
    @Schema(
        ...
        description = """
            By default, the memory ID is the name of the flow concatenated by the correlationId, this means that the same memory will be used by all tasks of the flow and its subflow.
            If you want to remove the memory eagerly (before expiration), you can set `drop: true` inside the last task of the flow so the memory is erased."""
    )
    @Builder.Default
