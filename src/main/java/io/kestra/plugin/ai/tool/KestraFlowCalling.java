...
public class KestraFlowCalling extends ToolProvider {
    ...
    public Map<ToolSpecification, ToolExecutor> tool(RunContext runContext) throws IllegalVariableEvaluationException {
        ...
        List<Label> executionLabels = MapUtils.nestedToFlattenMap(MapUtils.emptyOnNull((Map<String, Object>) runContext.getVariables().get("labels"))).entrySet().stream()
            ...
            .toList();
        List<Label> newLabels = runContext.render(inheritLabels).as(Boolean.class).orElse(false) ? new ArrayList<>(filterLabels(executionLabels, flowInterface)) : new ArrayList<>(systemLabels(executionLabels));
        if (labels != null) {
    ...
    static class KestraFlowToolExecutor implements ToolExecutor {
        ...
        private final List<Label> predefinedLabels;

        KestraFlowToolExecutor(DefaultRunContext runContext, FlowInterface flowInterface, Map<String, Object> predefinedInputs, List<Label> predefinedLabels) {
        ...
        public String execute(ToolExecutionRequest toolExecutionRequest, Object memoryId) {
            ...
            try {
                ...
                var labelList = ListUtils.emptyOnNull(labels).stream()
                    ...
                    .toList();
                var predefinedLabelsToAdd = predefinedLabels.stream().filter(l1 -> labelList.stream().noneMatch(l2 -> l1.key().equals(l2.key()))).toList();
